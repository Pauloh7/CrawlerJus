...
* Respeitar mais o Retry-After do header quando vem, e adicionar jitter no backoff pra parecer mais "humano"
* Colocar logs estruturados (com JSON) e métricas simples (quantas 429, tempo médio de resposta, hit rate do cache) pra facilitar debug quando o site mudar de novo
* Expandir os testes pra cobrir mais cenários ruins: token expirado, Redis down, resposta HTML no lugar de JSON, etc.
